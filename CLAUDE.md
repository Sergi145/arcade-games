...
- `.claude/agents/skin-designer` — visual-skins agent, two modes: audit (no game named — rescans `REAL_GAMES` in `components/real-game-registry.tsx` and refreshes the status table in `references/skins-audit.md`, Clásico/Neón/Retro per game) and implementation (a specific game + skin named — builds just that one, bootstrapping the shared `lib/skins.ts`/`SKIN_ENABLED_GAMES`/selector in `jugar-client.tsx` the first time, then updates the table). Scoped to skin-related files only (engines' color layer, the relevant `*-canvas.tsx`, the shared skin plumbing) — never game logic, Supabase tables, or other screens. Only builds the game/skin explicitly requested, never all games at once.
