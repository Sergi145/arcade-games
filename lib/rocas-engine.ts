...
const RADII = [0, 16, 30, 50] as const; // por tamaño 1, 2, 3
const SPEEDS = [0, 85, 55, 32] as const; // velocidad base por tamaño
const POINTS = [0, 100, 50, 20] as const; // puntos por tamaño
...
export function createRocasEngine(
  ...
  class Asteroid {
    ...
    split(): Asteroid[] {
      ...
      return [new Asteroid(this.x, this.y, nextSize), new Asteroid(this.x, this.y, nextSize)];
    }
  ...
  function reportState() {
    callbacks.onUpdate({ score, lives, level, gameOver: state === "gameover" });
  }
